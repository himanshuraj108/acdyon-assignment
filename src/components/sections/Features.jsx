...
function FeatureCard({ feature, index }) {
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-2xl p-7 border border-gray-200 ${c.border} transition-all duration-200 shadow-card hover:shadow-card-hover cursor-default`}
    >
      <div className={`w-11 h-11 rounded-xl ${c.bg} flex items-center justify-center mb-5`}>
        <Icon size={20} className={c.icon} strokeWidth={2} />
      </div>
      <h3 className="font-heading font-bold text-[18px] text-gray-900 mb-3 leading-snug">
        {feature.title}
      </h3>
      <p className="text-[14.5px] text-gray-500 leading-relaxed">{feature.description}</p>
    </motion.div>
  )
}
...
export default function Features() {
  return (
    <section className="py-24 lg:py-32 bg-gray-50/60 section-divider" id="platform">
